(()=>{
  ...
  function ensureStyles(doc){
    ...
    style.textContent=`
      ...
      .tiles .tile.ws-brax-hidden::after{content:'?';position:absolute;inset:0;display:grid;place-items:center;color:#fff0bd;font:1000 1rem Georgia,serif;text-shadow:0 2px 5px rgba(0,0,0,.45)}
      ...
      @media(max-width:430px){.ws-boss-ability-badge{min-height:33px;padding:6px 8px;margin-bottom:7px;font-size:.46rem}.ws-boss-ability-badge::before{width:22px;height:22px;flex-basis:22px;font-size:.68rem}.tiles .tile.ws-decoy::before{width:13px;height:13px;font-size:.42rem}}
  ...
  function installAbilityRuntime(doc){
    if(doc.getElementById('ws-boss-abilities-1-3-runtime-v2'))return;
    const runtime=doc.createElement('script');
    runtime.id='ws-boss-abilities-1-3-runtime-v2';
    runtime.textContent=`(()=>{
      if(window.__WS_BOSS_ABILITIES_1_3_V2__)return;
      window.__WS_BOSS_ABILITIES_1_3_V2__='${RELEASE}';
      ...
      const bossKey=()=>{
        let answer='';
        try{if(typeof window.WS_GET_BOSS_ANSWER==='function')answer=window.WS_GET_BOSS_ANSWER()||'';}catch{}
      ...
      function resetForKey(key){
        clearTransient();
        activeKey=key;
        kaiOrder=[];
        kaiAnimatedKey='';
        braxHiddenIndex=null;
        braxRevealAt=0;
        blackfinnDecoy='';
      }
      ...
      function cleanup(){
        activeKey='';
        clearTransient();
        document.querySelectorAll('.ws-boss-ability-badge').forEach(el=>el.remove());
      }
      ...
      function ensureBadge(level){
        ...
        if(!badge){
          ...
          badge.className='ws-boss-ability-badge';
      ...
      function ensureKai(key){
        ...
        if(!box)return;
        const tiles=Array.from(box.querySelectorAll('.tile[data-i]:not(.ws-decoy)'));
        if(tiles.length<2)return;
        if(!kaiOrder.length){
          kaiOrder=tiles.map(el=>String(el.dataset.i));
          if(kaiOrder.length%2===0)kaiOrder.reverse();else kaiOrder.push(kaiOrder.shift());
        }
        ...
        if(kaiAnimatedKey!==key){
          kaiAnimatedKey=key;
          kaiOrder.forEach((id,i)=>{
            const el=byIndex.get(id);if(!el)return;
            el.style.setProperty('--ws-shift',(i%2===0?6:-6)+'px');
            el.style.setProperty('--ws-rot',(i%2===0?2:-2)+'deg');
            el.classList.remove('ws-kai-shuffled');void el.offsetWidth;el.classList.add('ws-kai-shuffled');
          });
        }
      }

      function chooseBraxTarget(){
        const tiles=Array.from(document.querySelectorAll('.tiles .tile[data-i]:not(.used):not(.ws-decoy)'));
        if(!tiles.length)return null;
        return String(tiles[Math.floor(tiles.length/2)].dataset.i);
      }

      function ensureBrax(key){
        if(braxHiddenIndex===null){
          braxHiddenIndex=chooseBraxTarget();
          braxRevealAt=Date.now()+2800;
          if(braxHiddenIndex!==null){
            clearTimeout(braxTimer);
            braxTimer=setTimeout(()=>{
              if(activeKey!==key||!bossActive()||currentLevel()!==2)return;
              braxRevealAt=0;
              try{render();}catch{}
            },2820);
          }
        }
        const tile=braxHiddenIndex===null?null:document.querySelector('.tiles .tile[data-i="'+braxHiddenIndex+'"]');
        if(!tile)return;
        if(braxRevealAt>Date.now())tile.classList.add('ws-brax-hidden');
        else{tile.classList.remove('ws-brax-hidden');tile.classList.add('ws-brax-reveal');}
      }

      function chooseDecoy(){
        const used=new Set(sentenceUnits().map(v=>String(v).toUpperCase()));
        if(used.size){
          for(const word of ['ALWAYS','NEVER','CANNON','MOON','SECRET','ANCHOR','COMPASS'])if(!used.has(word))return word;
        }
      ...
      function ensureBlackfinn(){
        ...
        fake.textContent=blackfinnDecoy;
        fake.setAttribute('aria-label',blackfinnDecoy+', Köder');
        fake.addEventListener('click',ev=>{
          ...
          document.querySelector('.ws-decoy-note')?.remove();
          const note=document.createElement('div');note.className='ws-decoy-note';note.textContent='KÖDER!';
          document.querySelector('.card')?.appendChild(note);setTimeout(()=>note.remove(),650);
        });
        const children=Array.from(box.children);
        const at=Math.min(children.length,Math.max(1,Math.floor(children.length/2)));
        if(children[at])box.insertBefore(fake,children[at]);else box.appendChild(fake);
      }
      ...
      function abilityRender(){
        ...
        const key=bossKey();
        if(key!==activeKey)resetForKey(key);
        ensureBadge(level);
  ...
  function loadScript(id,src){
    return new Promise((resolve,reject)=>{
      ...
      script.id=id;
      script.async=false;
      script.src=src+(src.includes('?')?'&':'?')+'v='+RELEASE;
  ...
  function assertRuntimeContracts(){
    ...
    const checks=[
      ['boss-abilities-1-3',Boolean(win?.__WS_BOSS_ABILITIES_1_3_V2__)],
  ...
  async function bootstrapRuntime(){
    ...
    try{
      ...
      doc.documentElement.dataset.wsOuterRuntimeRelease=RELEASE;
      doc.documentElement.dataset.wsCoreRuntime='ready';
    }catch(err){
      document.documentElement.dataset.wsStableRuntime='failed';
      doc.documentElement.dataset.wsCoreRuntime='failed';
      console.error('Word Scramble runtime bootstrap failed',err);
    }
    return true;
  }
  ...
  function watchUntilReady(){
    ...
    const doc=getFrameDoc();
    if(doc&&doc!==observedDoc){
      observedDoc=doc;
      readyObserver?.disconnect();
      const target=doc.getElementById('game')||doc.documentElement;
      if(target){
        readyObserver=new MutationObserver(()=>{bootstrapRuntime();});
        readyObserver.observe(target,{childList:true,subtree:true});
      }
    }
    if(attempts<120){
      clearTimeout(readyTimer);
      readyTimer=setTimeout(watchUntilReady,100);
    }else{
      document.documentElement.dataset.wsStableRuntime='failed';
      console.error('Word Scramble runtime bootstrap timed out before the game frame became ready');
    }
  }
